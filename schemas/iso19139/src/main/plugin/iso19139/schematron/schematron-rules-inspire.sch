...
-->
	...
	<sch:pattern>
		...
		<sch:rule context="//srv:SV_ServiceIdentification/srv:operatesOn[@uuidref != '']">
			<sch:let name="metadataUUID" value="../../../gmd:fileIdentifier/gco:CharacterString"/>
			<sch:let name="coupledResourceUUID" value="current()/@uuidref"/>

			<!-- check for circular reference exception -->
			<sch:assert test="$coupledResourceUUID != $metadataUUID">
				<sch:value-of select="$loc/strings/alert.M81/div"/>
			</sch:assert>
			<sch:report test="$coupledResourceUUID != $metadataUUID">
				<sch:value-of select="$loc/strings/report.M81/div"/>
			</sch:report>
	...
	<sch:pattern>
		...
		<sch:rule context="//gmd:MD_DataIdentification|
			...
			-->
            <sch:let name="thesaurusURL" value="concat('file:///', replace(replace($thesaurusDir,' ','%20'),'\\','/'), '/external/thesauri/theme/inspire-theme.rdf')"/>
	...
	<sch:pattern>
		...
		<sch:rule context="//srv:SV_ServiceIdentification|//*[@gco:isoType='srv:SV_ServiceIdentification']">
			<!-- Check that INSPIRE service taxonomy is available.
				Use INSPIRE thesaurus available on SVN to check keywords in all EU languages.
			-->
			<sch:let name="inspire-thesaurus" value="document(concat('file:///',replace(replace($thesaurusDir,' ','%20'),'\\','/'), '/external/thesauri/theme/inspire-service-taxonomy.rdf'))"/>
	...
	<sch:pattern>
		...
		<sch:rule context="/gmd:MD_Metadata[gmd:identificationInfo/gmd:MD_DataIdentification
			or gmd:identificationInfo/*/@gco:isoType = 'gmd:MD_DataIdentification']">
			<sch:let name="count" value="count(gmd:dataQualityInfo/*/gmd:lineage)"/>
			<sch:assert test="$count > 0">$loc/strings/alert.M43/div</sch:assert>
			<sch:report test="$count > 0">$loc/strings/report.M43/div</sch:report>
		</sch:rule>
